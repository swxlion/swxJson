...
class JsonParser
{
	...
	JsonPtr enterObject()
	{
		if (_wantKey || _wantSemicolon || _wantComma)
			throw JSON_ERROR_MSG(JosnInvalidContentError, "Json parser: content error, '{' at improper place.");
	...
	void fillValue(JsonPtr node)
	{
		if (_nodeStack.top()->addNode(_key, node) == false)
			throw JSON_ERROR_FMT(JosnInvalidContentError, "Json parser: content error, reduplicated key: %s.", _key.c_str());
	...
	JsonPtr processString()
	{
		if(_wantComma || _wantSemicolon)
				throw JSON_ERROR_MSG(JosnInvalidContentError, "Json parser: content error.");
	...
	{
		if (_wantComma == false)
			throw JSON_ERROR_MSG(JosnInvalidContentError, "Json parser: content error, ',' at error position.");
	...
	JsonPtr processNumber()
	{
		...
		if (errno || endptr != _pos)
			throw JSON_ERROR_MSG(JosnInvalidContentError, "Json parser: content error, invalid number or huge number.");
