...
//-        Assistant Functions         -//
//======================================//
std::ostream& swxJson::operator << (std::ostream& os, const Json& node)
{
	return node.output(os);
}

std::ostream& swxJson::operator << (std::ostream& os, const JsonPtr node)
{
	return node->output(os);
}
...
JsonPtr Json::createNode(const std::string& path, const std::string& delim, bool throwable)
{
	...
	for (; i < sections.size(); i++)
	{
		...
		if (next == nullptr)
		{
			...
			{
				if (throwable)
					throw JSON_ERROR_FMT(JsonNodeTypeMissMatchError, "Section %d (%s)(base 0) in path is not object.", (int)i, sections[i].c_str());
...
enum Json::ElementType Json::type(const std::string& path, const std::string& delim) throw(JosnNodeNotExistError)
{
	...
	throw JSON_ERROR_MSG(JosnNodeNotExistError, "Node is not exist.");
}
...
Json& Json::operator [] (int index) throw(JsonNodeTypeMissMatchError)
{
	...
	if (_type != JSON_Array)
		throw JSON_ERROR_MSG(JsonNodeTypeMissMatchError, "Current node is not an array.");
